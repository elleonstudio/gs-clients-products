...
def recognize_photos_batch(photo_urls, catalog_text):
    prompt = f"Каталог:\n{catalog_text}\n\nЯ отправляю фото. Найди ID товаров. Верни СТРОГО JSON массив строк."
...
def parse_logistics_with_kimi(text, photo_url):
    prompt = "Ты логист. Вытащи из данных ПОЛНУЮ коробку: pcs_per_ctn, gw_kg, length, width, height. Верни СТРОГО JSON."
    content = [{"type": "text", "text": prompt}]
...
async def generate_final_invoice(update: Update, context: ContextTypes.DEFAULT_TYPE, uid: int, page_id=None):
    ...
    s.update({"subtotal_cny": subtotal, "comm_amd": c_amd, "tot_amd": tot_amd, "client_rate": c_rate, "inv_text": inv_text})
...
async def process_cargo_items(update: Update, context: ContextTypes.DEFAULT_TYPE, uid: int):
    ...
    while idx < len(d["items"]) and ("boxes" in d["items"][idx] or d["items"][idx].get("pack_type") == "inset"): idx += 1
    ...
    kb = []
    if item.get("cm"):
        kb.append([InlineKeyboardButton("⚡️ База Notion", callback_data="cg_use_db")])
    await context.bot.send_message(chat_id=update.effective_chat.id, text=text + "Введи данные или перешли ответ китайца:", parse_mode='HTML', reply_markup=InlineKeyboardMarkup(kb) if kb else None)

async def finish_cargo_dims(update: Update, context: ContextTypes.DEFAULT_TYPE, uid: int):
    d = cargo_drafts[str(uid)]
    t_w = t_v = t_p = 0
    for i in d["items"]:
        if i.get("pack_type") == "inset": continue
    ...
    d.update({"t_weight": t_w, "t_vol": t_v, "t_pieces": t_p, "density": int(t_w/t_v) if t_v > 0 else 0, "state": "CARGO_WAIT_TARIFF_CG"})
    await context.bot.send_message(chat_id=update.effective_chat.id, text=f"📊 <b>ИТОГ:</b> {t_w:.1f}кг | {t_v:.2f}м³ | {t_p} мест. Плотность: {d['density']}\n👉 Напиши Тариф Карго ($/кг):", parse_mode='HTML')
    ...
    if uid in user_sessions:
        ...
        if s["state"] == "COLLECTING":
            ...
            for i, item in enumerate(s["items"]):
                if text in item['name'] or text == str(i+1):
                    s["current_item_index"], item["qty_confirmed"], item["shipping"], found = i, False, None, True
                    break
            if found: await ask_next_question(update, context, uid)
            else: await update.message.reply_text("❌ Не нашел такой товар.")
            return
        ...
        if d["state"] == "CARGO_WAIT_DIMS":
            ...
            tot_amd = int(t_w * t_cl * r_amd)
            net_profit = int((t_cl - t_cg) * t_w * r_amd)
            msg_client = f"🚛 <b>CARGO INVOICE: {d['client'].upper()}</b>\nВес: {t_w:.1f}кг | К ОПЛАТЕ: {tot_amd:,} AMD"
            msg_admin = f"💼 <b>ВНУТРЕННИЙ РАСЧЕТ ({d['client'].upper()}):</b>\n💰 ПРИБЫЛЬ: {net_profit:,} AMD"
            await update.message.reply_text(msg_client, parse_mode='HTML')
            await update.message.reply_text(msg_admin, parse_mode='HTML', reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📊 Excel", callback_data=f"cgexcel_{save_to_notion_cache(d)}"), InlineKeyboardButton("📑 Airtable", callback_data=f"cargodb_{save_to_notion_cache(d)}")]]))
...
async def process_kimi_logistics_result(update: Update, context: ContextTypes.DEFAULT_TYPE, uid: int, res: dict):
    ...
    if not res: return await update.message.reply_text("❌ ИИ не понял данные.")
    pcs = int(res.get('pcs_per_ctn', 1)) or 1
    t_qty = int(item['qty'])
    res['full_cartons'], res['remainder'] = t_qty // pcs, t_qty % pcs
    d["temp_kimi"] = res
    msg = f"🧠 <b>Kimi [{item['name']}]:</b>\nВ коробке {pcs} шт | Вес {res['gw_kg']}кг | {res['length']}x{res['width']}x{res['height']}\n✅ Полных: {res['full_cartons']} | ⚠️ Остаток: {res['remainder']}"
    await update.message.reply_text(msg, parse_mode='HTML', reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Сохранить", callback_data="cg_accept_kimi"), InlineKeyboardButton("❌ Ошибка", callback_data="cg_reject_kimi")]]))
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("editinit_"):
        ...
        await q.message.reply_text("Введи номер товара из списка или его название для изменения:")
    
    elif data.startswith("airtable_"):
        d = get_from_notion_cache(data.split("_")[1])
        payload = {"records": [{"fields": {"Клиент": d["client"], "Заказ": d["inv_text"] + f"\nИТОГО: {d['tot_amd']} AMD"}}], "typecast": True}
        ...
        await q.message.reply_text("✅ Записано в Airtable!")
        ...
        else: await q.message.reply_text(f"Введи габариты для остатка ({r['remainder']} шт):")
        await process_cargo_items(update, context, uid)
        ...
        # Улучшенный парсинг габаритов из Notion
        dims = [float(s) for s in i["cm"].replace('x',' ').replace('*',' ').replace('х',' ').split() if s.replace('.','').isdigit()]
        i["boxes"] = [{"qty": int(i['qty'])//i['pcs_ctn'], "w": i["gw_kg"], "l": dims[0], "w_dim": dims[1], "h": dims[2]}]
